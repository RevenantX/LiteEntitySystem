...
namespace LiteEntitySystem.Internal
{
    ...
    internal abstract unsafe class ValueTypeProcessor
    {
        ...
        internal abstract void SetFrom(InternalBaseClass obj, int offset, byte* data, string fieldName);
        ...
        internal abstract void SetInterpolation(InternalBaseClass obj, int offset, byte* prev, byte* current,
            float fTimer);

        internal abstract void LoadHistory(InternalBaseClass obj, int offset, byte* tempHistory, byte* historyA,
            byte* historyB, float lerpTime);
    ...
    internal unsafe class ValueTypeProcessor<T> : ValueTypeProcessor where T : unmanaged
    {
        ...
        internal override void SetInterpolation(InternalBaseClass obj, int offset, byte* prev, byte* current,
            float fTimer) =>
        ...
        internal override void LoadHistory(InternalBaseClass obj, int offset, byte* tempHistory, byte* historyA,
            byte* historyB, float lerpTime)
        {
        ...
        internal override void SetFrom(InternalBaseClass obj, int offset, byte* data, string fieldName)
        {
            ref var syncVar = ref RefMagic.RefFieldValue<SyncVar<T>>(obj, offset);
            T oldValue = syncVar.Value;
            T newValue = *(T*)data;
            
            if (!Utils.FastEquals(ref oldValue, ref newValue))
            {
                syncVar.SetDirect(newValue);
                SyncVarCallbackRegistry.TryInvoke(obj.GetType(), fieldName, obj, newValue);
            }
        }
    ...
    {
        internal override unsafe void SetInterpolation(InternalBaseClass obj, int offset, byte* prev, byte* current,
            float fTimer) =>
            RefMagic.RefFieldValue<SyncVar<int>>(obj, offset)
                .SetDirect(Utils.Lerp(*(int*)prev, *(int*)current, fTimer));

        internal override unsafe void LoadHistory(InternalBaseClass obj, int offset, byte* tempHistory, byte* historyA,
            byte* historyB, float lerpTime)
        {
    ...
    internal class ValueTypeProcessorLong : ValueTypeProcessor<long>
    {
        internal override unsafe void SetInterpolation(InternalBaseClass obj, int offset, byte* prev, byte* current,
            float fTimer) =>
            RefMagic.RefFieldValue<SyncVar<long>>(obj, offset)
                .SetDirect(Utils.Lerp(*(long*)prev, *(long*)current, fTimer));
    ...
    internal class ValueTypeProcessorFloat : ValueTypeProcessor<float>
    {
        internal override unsafe void SetInterpolation(InternalBaseClass obj, int offset, byte* prev, byte* current,
            float fTimer) =>
            RefMagic.RefFieldValue<SyncVar<float>>(obj, offset)
                .SetDirect(Utils.Lerp(*(float*)prev, *(float*)current, fTimer));
    ...
    internal class ValueTypeProcessorDouble : ValueTypeProcessor<double>
    {
        internal override unsafe void SetInterpolation(InternalBaseClass obj, int offset, byte* prev, byte* current,
            float fTimer) =>
            RefMagic.RefFieldValue<SyncVar<double>>(obj, offset)
                .SetDirect(Utils.Lerp(*(double*)prev, *(double*)current, fTimer));
    ...
    internal unsafe class UserTypeProcessor<T> : ValueTypeProcessor<T> where T : unmanaged
    {
        ...
        internal override void SetInterpolation(InternalBaseClass obj, int offset, byte* prev, byte* current,
            float fTimer) =>
            RefMagic.RefFieldValue<SyncVar<T>>(obj, offset)
                .SetDirect(_interpDelegate?.Invoke(*(T*)prev, *(T*)current, fTimer) ?? *(T*)prev);
