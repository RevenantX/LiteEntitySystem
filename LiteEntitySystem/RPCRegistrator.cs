...
namespace LiteEntitySystem
{
    ...
    public struct RemoteCall<T1, T2> where T1 : unmanaged where T2 : unmanaged
    {
        internal Delegate CachedAction;
        internal static RemoteCall<T1, T2> Create<TClass>(ValueSpanAction<TClass, T1, T2> action) => new() { CachedAction = action };
    ...
    public readonly ref struct RPCRegistrator
    {
        ...
        /// <param name="remoteCallHandle">output handle that should be used to call rpc</param>
        /// <param name="flags">RPC execution flags</param>
        public unsafe void CreateRPCAction<TEntity, T1, T2>(ValueSpanAction<TEntity, T1, T2> methodToCall, ref RemoteCall<T1, T2> remoteCallHandle, ExecuteFlags flags) 
            where T1 : unmanaged
            where T2 : unmanaged
            where TEntity : InternalEntity
        {
            ushort rpcId = (ushort)_calls.Count;
            _calls.Add(new RpcFieldInfo(RemoteCall<T1, T2>.CreateMCD(methodToCall)));
            if (remoteCallHandle.CachedAction != null)
                return;
            remoteCallHandle = RemoteCall<T1, T2>.Create<InternalEntity>((e,v1,v2) =>
            {
                if (e.EntityManager.IsServer)
                {
                    if (flags.HasFlagFast(ExecuteFlags.ExecuteOnServer))
    ...
    public ref struct SyncableRPCRegistrator
    {
        ...
        public unsafe void CreateClientAction<TSyncField, T1, T2>(ValueSpanAction<TSyncField, T1, T2> methodToCall, ref RemoteCall<T1, T2> remoteCallHandle)
            where T1 : unmanaged
            where T2 : unmanaged
            where TSyncField : SyncableField
        {
            ushort rpcId = _internalRpcCounter++;
            _calls.Add(new RpcFieldInfo(_syncableOffset, RemoteCall<T1, T2>.CreateMCD(methodToCall)));
            if (remoteCallHandle.CachedAction != null)
                return;
            remoteCallHandle = RemoteCall<T1, T2>.Create<SyncableField>((s, value1, value2) =>
            {
