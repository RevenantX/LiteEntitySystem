...
namespace LiteEntitySystem.Generator
{
    ...
    public class BindingGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            var staticActionsText = new StringBuilder();
...
using LiteEntitySystem.Internal;");
                ...
                data = data.Slice(_target.IsRemoteControlled ? Helpers.SizeOfStruct<{syncVarGenericArg}>() : Helpers.ReadStructAndReturnSize(data, out {fieldName}.Value));");                            
                            ...
                            if (bindOnChangeAttr != null)
                            {
                                ...
                                string bindMethod = bindOnChangeAttr.ConstructorArguments[0].Value.ToString();
                                //load if differenct
                                readChangedText.Append(@$"
                    ...
                    {{
                        ...
                        fieldsData.OnChange(
                            _RPCCache{className}{bindMethod}, 
                            _target, 
                    ...
                    SliceBySize<{syncVarGenericArg}>(ref fieldsData.RawData);");
                                staticActionsText.Append($@"
            private static readonly SpanAction<InternalEntity, byte> _RPCCache{className}{bindMethod} = (e, d) => (({className})e).{bindMethod}(d.ReadStruct<{syncVarGenericArg}>());");
                            }
                            else
                            {
                                readChangedText.Append(@$"
